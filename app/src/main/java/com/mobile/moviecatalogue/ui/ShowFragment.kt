...
import com.mobile.moviecatalogue.databinding.FragShowBinding
import com.mobile.moviecatalogue.viewmodel.ShowViewModel
import dagger.hilt.android.AndroidEntryPoint

@AndroidEntryPoint
class ShowFragment : BaseFragment() {
    private var _binding: FragShowBinding? = null
    private val binding get() = _binding as FragShowBinding
    ...
    override fun onCreateView(
        ...
        _binding = FragShowBinding.inflate(inflater, container, false)
    ...
    private fun setupRV() {
        ...
        with(binding.fragShowListRv) {
            layoutManager = LinearLayoutManager(context)
            adapter = ShowPagedRVAdapter(
                R.layout.vh_show,
                clickListener
            )
    ...
    private fun observeNavigationEvent() {
        viewmodel.navigateToDetails.observe(viewLifecycleOwner, Observer {
            ...
            val action = ShowFragmentDirections.actionShowFragmentToShowDetailsFragment2(it)
    ...
    private fun observeLoadingState() {
        ...
        viewmodel.isLoading.observe(viewLifecycleOwner, Observer {
            ...
            val toast = makeToast(getString(R.string.toast_load_more_data_text)).apply {
                show()
            }
        ...
        viewmodel.errorLoading.observe(viewLifecycleOwner, Observer {
            if (it == false) return@Observer
            val toast = makeToast(getString(R.string.toast_error_loading_text)).apply {
                show()
            }
            dismissToastAfterDelay(toast)
        })
